...
const [fontsLoaded, setFontsLoaded] = useState(() => {
  ...
  if (!('fonts' in document)) return true; // Support older browsers
  ...
  useEffect(() => {
  // 2. Only add the listener if fonts aren't loaded yet
  if (typeof window !== 'undefined' && 'fonts' in document && document.fonts.status !== 'loaded') {
    let active = true;
